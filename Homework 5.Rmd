...
ta1 <- Wong %>%
  group_by(sex) %>%
   summarise(mediandays = median(days, na.rm=TRUE),
            firstqdays  =   quantile(days,0.25, na.rm=TRUE),
            thirdrdqdays = quantile(days, 0.75, na.rm=TRUE),
             medianduration = median(duration, na.rm=TRUE),
            firstqduration =   quantile(duration, 0.25, na.rm=TRUE),
            thirdqduration = quantile(duration, 0.75, na.rm=TRUE))

knitr::kable(ta1,
              col.names=c("SEX",
                         "Median Days","1st quantile Days",
                         "3rd quantile days",
                         "Median Duration","1st quantile Duration",
                         "3rd quantile days"),
             caption="Summary Statistics by Sex, Days, and Duration")
```
...
t2 <- Wong %>%
  group_by(sex) %>%
   summarise(avgperformance = mean(piq, na.rm=TRUE),
            sdperformance  =   sd(piq, na.rm=TRUE),
            avgverbal = mean(viq, na.rm=TRUE),
            sdverbal  =   sd(viq, na.rm=TRUE))

knitr::kable(t2,
             col.names=c("Sex",
                         "Mean Performance","SD Performance",
                         "Mean Verbal","SD Verbal"),

             caption="Summary Statistics by Sex, PerformanceIq and Verbal IQ")
```
...
#Load DT
library(DT)
#Create interactive table from html widget with filter option turned on
datatable(Wong, filter = 'top', options = list(
...
Rmd file can be found at <https://github.com/amonda/N741interactive>, entitled "Homework 5.rmd". 
